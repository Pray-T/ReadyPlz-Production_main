package io.readyplz.readyplz.repository;
...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query(
        value = "select new io.readyplz.readyplz.dto.summary.MessageSummaryDTO(m.id, m.content, m.createdAt, m.isRead, s.id, s.nickname, r.id, r.nickname) " +
    ...
    Page<io.readyplz.readyplz.dto.summary.MessageSummaryDTO> findBySenderId(@Param("memberId") Long memberId, Pageable pageable);
    ...
    Page<io.readyplz.readyplz.dto.summary.MessageSummaryDTO> findByReceiverId(@Param("memberId") Long memberId, Pageable pageable);
    ...
    Page<io.readyplz.readyplz.dto.summary.MessageSummaryDTO> findConversation(Long memberId1, Long memberId2, Pageable pageable);
    ...
    /** 1:1 대화에서 가장 오래된 메시지 ID 페이지 (정렬은 Pageable의 Sort 사용). */
    @Query("SELECT m.id FROM Message m WHERE (m.sender.id = :a AND m.receiver.id = :b) "
            + "OR (m.sender.id = :b AND m.receiver.id = :a)")
    Page<Long> findOldestMessageIdsInConversation(@Param("a") Long memberIdA, @Param("b") Long memberIdB,
            Pageable pageable);
