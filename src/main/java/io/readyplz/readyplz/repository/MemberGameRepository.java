...
public interface MemberGameRepository extends JpaRepository<MemberGame, Long> { // 엔티티 클래스와 ID 타입을 지정
    ...
    List<io.readyplz.readyplz.dto.summary.MemberSummaryDTO> findMemberSummariesByGameId(@Param("gameId") Long gameId);

    /** 특정 게임을 가진 회원들을 요약 DTO로 조회하되 특정 사용자 제외 */
    @Query("select new io.readyplz.readyplz.dto.summary.MemberSummaryDTO(m.id, m.username, m.nickname, m.country) " +
           "from MemberGame mg join mg.member m where mg.game.id = :gameId and m.id <> :currentUserId")
    List<io.readyplz.readyplz.dto.summary.MemberSummaryDTO> findMemberSummariesByGameIdExcludingUser(@Param("gameId") Long gameId,
                                                                                                       @Param("currentUserId") Long currentUserId);

     //여러 게임에 대해, 각 게임을 가진 회원 정보를 DTO프로젝션을 통해 해결 (N+1 방지) 
    @Query("select new io.readyplz.readyplz.dto.summary.GameUserSummaryDTO(g.id, m.id, m.username, m.nickname, m.country) " +
           "from MemberGame mg join mg.member m join mg.game g where g.id in :gameIds")
    List<io.readyplz.readyplz.dto.summary.GameUserSummaryDTO> findGameUserSummariesByGameIds(@Param("gameIds") List<Long> gameIds);
}