...
public class GameController {
    ...
    // 이 메서드는 사용자의 게임 컬렉션 페이지를 렌더링하는 역할을 합니다. 사용자가 가진 게임 목록, 게임 검색 기능 및 결과, 그리고 각 게임을 가진 다른 사용자 목록을 화면에 표시하기 위한 데이터를 준비합니다.
    ...
    public String gameCollection(@RequestParam(value = "page", defaultValue = "0") int page,
        ...
        if (!userGameIds.isEmpty()) {
            Map<Long, List<io.readyplz.readyplz.dto.summary.MemberSummaryDTO>> sameGameUsersMap = memberGameService.getSameGameUsersForAllGames(userGameIds, member.getId());
        ...
        if (search != null && !search.trim().isEmpty()) {
            ...
            //검색된 각 게임에 대해, 현재 사용자가 이미 가지고 있는 게임인지 여부를 나타내는 boolean 플래그를 설정합니다. 이는 뷰에서 "추가" 또는 "삭제" 버튼을 동적으로 표시하는 데 사용됩니다.
            games.forEach(game -> {
