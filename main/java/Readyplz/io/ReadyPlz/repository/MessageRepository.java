...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM message WHERE id IN (" +
            "SELECT id FROM (" +
            "SELECT id, ROW_NUMBER() OVER (ORDER BY created_at ASC) AS rn FROM message" +
            ") AS temp WHERE temp.rn <= ?1" +
            ")", nativeQuery = true)
    void deleteOldestMessages(int deleteLimit);
